#!/usr/bin/env node
/**
 * Local end-to-end test: loads the real extension through OMP while the
...
if (!OMP_BIN) {
  throw new Error("OMP_BIN is unset and omp was not found on PATH")
}

const ompCheck = spawnSync(OMP_BIN, ["--help"], { stdio: "ignore" })
if (ompCheck.error || ompCheck.status !== 0) {
...
async function runRpcQuery(timeoutMs = 30_000) {
  const child = spawn(
    OMP_BIN,
    ["--mode", "rpc", "--extension", EXT_PATH, "--model", TEST_MODEL_SELECTOR],
    {
      cwd: PROJECT_DIR,
      env,
...
try {
  ...
  console.log("[omp-local] print mode through real extension and mock API")
  requestCount = 0
